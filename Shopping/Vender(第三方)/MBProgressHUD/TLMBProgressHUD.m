...
@implementation TLMBProgressHUD
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	TLMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	TLMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [TLMBProgressHUD allHUDsForView:view];
	for (TLMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (TLMBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[TLMBRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[TLMBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[TLMBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(TLMBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
...
@implementation TLMBRoundProgressView
...
@implementation TLMBBarProgressView
